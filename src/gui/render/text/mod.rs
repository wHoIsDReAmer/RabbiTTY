...
pub(super) struct TextPipelineData {
    ...
    line_min_y: f32,
    cell_advance: f32,
    /// Keyed by cell size too: these are drawn to the cell, so they change with it.
...
impl TextPipelineData {
    ...
    pub(super) fn push_pane(
        ...
        for cell in cells {
            ...
            let info = if drawn_to_cell {
                ...
                {
                    Some(info) => info,
                    // Nothing in the range is drawn for this one; the font keeps it.
            ...
            let pos = if drawn_to_cell && info.bearing == [0.0, 0.0] {
                // Placed on the cell itself, not on the baseline, so the shape
                // meets its neighbours exactly.
    ...
    fn get_or_insert_synthetic(
        ...
        // Same layout the subpixel path uploads, with all three channels equal so
        // the shape carries no colour fringe.
